    ...
    K: Clone,
{
    ...
    // TODO(calintat): This and the one above should be a single function.
    pub fn forked_explode<R, F>(self, f: F) -> Result<GraphBuilder<R>, DimensionError>
    where
        F: Fn(Height, K) -> R + Copy,
    {
        use Height::{Regular, Singular};
        ...
        for (key, node) in self.nodes {
            ...
            for (i, cospan) in node.cospans().iter().enumerate() {
                edges_exploded.push((
                    nodes_index_start + Regular(i).to_int(),
                    nodes_index_start + Singular(i).to_int(),
                    cospan.forward.clone(),
                ));

                edges_exploded.push((
                    nodes_index_start + Regular(i + 1).to_int(),
                    nodes_index_start + Singular(i).to_int(),
                    cospan.backward.clone(),
                ));
            }
        ...
        for (source, target, rewrite) in self.edges {
            ...
            for source_height in 0..(source_size - 1) / 2 {
                let target_height = rewrite.singular_image(source_height);
                edges_exploded.push((
                    source_index + Singular(source_height).to_int(),
                    target_index + Singular(target_height).to_int(),
                    rewrite.slice(source_height),
                ));
            }
            ...
            for target_height in 0..(target_size + 1) / 2 {
                let source_height = rewrite.regular_image(target_height);
                edges_exploded.push((
                    source_index + Regular(source_height).to_int(),
                    target_index + Regular(target_height).to_int(),
                    Rewrite::identity(self.dimension - 1),
                ));
            }
        }
